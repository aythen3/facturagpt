...
export const PricingCard = ({ title, price, selectedCard, setSelectedCard, index }) => {
  return (
    <div
      onClick={() => setSelectedCard(index)}
      className={`${styles.card} ${selectedCard ? styles.selectedCard : ""}`}
    >
      <div className={`${styles.cardHeader} ${selectedCard ? styles.selectedCardHeader : ""}`}>
        {title}
      </div>
      <div className={styles.cardBody} >
        <p  className={`${styles.price} ${selectedCard ? styles.selectedPrice : ""}`}>{price}€</p>
        <span  className={styles.subText}>POR DOCUMENTO</span>
        <button className={`${styles.button} ${selectedCard ? styles.selectedButton : ""}`}>
          Buy Now
        </button>
      </div>
    </div>
